...
export function AddContactModal() {
  ...
  if (!isOpen) return (
    <button onClick={() => setIsOpen(true)} className="w-full md:w-auto bg-[#bdfd2c] text-slate-950 px-6 py-3 rounded-2xl font-black text-sm hover:scale-105 transition-all shadow-[0_10px_20px_rgba(189,253,44,0.2)] uppercase tracking-tighter">
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] w-full max-w-md space-y-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        <h2 className="text-2xl font-black tracking-tighter text-[#bdfd2c] uppercase italic">Nuevo Prospecto</h2>
        <form action={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button type="submit" disabled={loading} className="flex-1 bg-[#bdfd2c] text-slate-950 font-black py-4 rounded-2xl text-sm shadow-[0_0_20px_rgba(189,253,44,0.3)] disabled:opacity-50">
