...
export function AddExpenseModal({ clubs }: { clubs: any[] }) {
  ...
  if (!isOpen) return (
    <button 
      ...
      className="inline-flex shrink-0 items-center justify-center whitespace-nowrap bg-[var(--color-bg-card)] border border-[var(--color-border)] text-rose-400 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-500/10 hover:border-rose-500/30 transition-all shadow-lg gap-2"
    >
  ...
  return createPortal(
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-[var(--color-bg-page)]/90 backdrop-blur-sm" onClick={() => setIsOpen(false)} />
      <div className="relative bg-[var(--color-bg-card)] border border-[var(--color-border)] p-8 rounded-[2rem] w-full max-w-md shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <form action={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select name="category" required className="w-full bg-[var(--color-bg-page)] border border-[var(--color-border)] rounded-xl p-3 text-sm text-[var(--color-text-heading)] font-bold mt-1 outline-none">
                ...
                <option value="torneo_inscripcion">Torneo o cancha abierta — Inscripción</option>
                <option value="torneo_premios">Torneo o cancha abierta — Premios</option>
                <option value="torneo_logistica">Torneo o cancha abierta — Logística</option>
              </select>
            </div>
          </div>
          ...
          <div>
            <label className="text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest ml-1">Concepto</label>
